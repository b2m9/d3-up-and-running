...
(function () {
  ...
  function plotChart (ctx, data, x, y, c, h) {
    ...
    svg.call(d3.zoom().on('zoom', function () {
      y = d3.event.transform.rescaleY(scale.y)
      d3.select('.axis--y')
        .call(axes.y.scale(y))
      d3.selectAll('.bar')
        .attr('y', function (d) { return y(d) })
  ...
  function plotAxes (ctx, x, y) {
    axes.x = d3.axisBottom().scale(x).tickFormat('').tickSizeInner(4)
      .tickSizeOuter(0)
    axes.y = d3.axisLeft().scale(y)
